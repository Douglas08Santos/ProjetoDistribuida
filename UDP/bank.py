...
while ok:
    ...
    if menuop == '2':
        ...
        while ok2:
            ...
            #Transferência
            if opmenu == '3':
            	consultarOrigem = input("\nInforme o codigo da conta em que deseja retirar a quantia(Conta origem): ")
            	consultarOrigemV = consultarOrigem in Contas['cod']

            	consultaDestino = input("\nInforme o codigo da conta de que deseja inserir a quantia(Conta destino): ")
            	consultaDestinoV = consultaDestino in Contas['cod']

            	if consultarOrigemV == True and consultaDestinoV == True:
            		posOrigem = Contas['cod'].index(consultarOrigem)
            		posDestino = Contas['cod'].index(consultaDestino)

            		saque = input("\nInforme o valor para ser transferido: ")
            		saque = float(saque)
            		ValorOrigem = Contas['saldo'][posOrigem]
            		ValorDestino = Contas['saldo'][posDestino]

            		if ValorOrigem > saque:
            			ValorOrigem = ValorOrigem - saque
            		...
            		if consultarOrigemV == False:
            			print("Erro na conta de origem")
            		else:
            			print("Erro na conta de destino")
